...
const UserList = ({ contacts }) => {
  ...
  const [contactsList, setContactsList] = useState(contacts?.combinedResults);
  ...
  useEffect(() => {
    if (requestStatus === 'success' || requestStatus === 'error') {
     ...
     (
              ...
              <button styles={{justifySelf:"flex-end",fontSize:"0.8rem", margin:"0.51rem"}} onClick={() =>deleteContact(contact)}>メ</button>
            {contact.name && (
              <div>
